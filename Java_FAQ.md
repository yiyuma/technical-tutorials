...
- Fuction<T,R> stellt eine Funktion dar, die ein Input akzeptiert und ein Ergebnis erzeugt. T ist Datentyp für Argument. R ist Datentyp für Ergebnis. Functional Method ist R apply(T t).
- Consumer<T> stellt eine Operation dar, die ein Input akzeptiert und kein Ergebnis zurückgibt. T ist Datentyp für Argument. Functional Mehtod ist void accept(T t).
- Predicate<T> stellt ein Predicate(boolean values Function) dar, das ein Argument akzeptiert und ein boolean-Wert zurückgibt. T ist Datentyp für Argument. Functional Method ist boolean test(T).
- Supplier<T> stellt ein Supplier dar, die kein Argument braucht und ein Ergebnis erzeugt. T ist Datentyp für Ergebnis. Functional Method ist T get().<br>
